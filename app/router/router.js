...
import Alarm from "../components/alarm/index.js"
import User from "../components/user/index.js"
import Detail1 from "../components/detail1.js"
import Detail2 from "../components/detail2.js"
import Login from "../components/login/index.js"

const HomeIcon = require('../../resources/syb.png');
const SearchIcon = require('../../resources/rlsbg.png');
const MainIcon = require('../../resources/wdb.png');
const AlarmIcon = require('../../resources/bjjlb.png');

const MyTab = TabNavigator({
    Index: {
        screen: Index,
        navigationOptions:() =>NavigationOptions('首页','首页',HomeIcon),
    },
    Face: {
        screen: Face,
        navigationOptions:() =>NavigationOptions('人脸识别','人脸识别',SearchIcon),
    },
    Alarm:{
        screen:Alarm,
        navigationOptions: ()=> NavigationOptions('报警记录','报警记录',AlarmIcon),
    },
    User:{
        screen:User,
        navigationOptions: ()=> NavigationOptions('我的','我的',MainIcon),
    },
},{
    ...
    initialRouteName:'Face', // 设置默认的页面组件
    backBehavior:'none', // 按 back 键是否跳转到第一个Tab(首页)， none 为不跳转
    tabBarOptions:{
        // iOS属性
        // 因为第二个tabbar是在页面中创建的，所以前景色的设置对其无效，当然也可以通过设置tintColor使其生效
        activeTintColor:'#0066FF', // label和icon的前景色 活跃状态下（选中）。
        inactiveTintColor:'#8F9094', // label和icon的前景色 不活跃状态下(未选中)。
...
const NavigationOptions = (headertitle,tablabel,Ticon) =>{
        return {
            // StackNavigator 属性部分

            // title:'Index', 同步设置导航和tabbar文字,不推荐使用
            headerTitle:headertitle, // 只会设置导航栏文字,
            ...
            tabBarIcon: (({tintColor,focused}) => {
                return(
                    <Image
                        source={!focused ? Ticon : Ticon}
                        style={[{height:20,width:20 }, {tintColor: tintColor}]}
                    />
                )
            }), // 设置标签栏的图标。需要单独设置。
            tabBarLabel:tablabel, // 设置标签栏的title。推荐这个方式。
        }
}